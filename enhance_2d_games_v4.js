const fs = require('fs');
const path = require('path');

const gamesDir = path.join(__dirname, 'games');
const games = fs.readdirSync(gamesDir).filter(f => f.endsWith('.html'));

const v4Script = `
            // ==========================================
            // SUPERCHARGED PARTICLE POPPER V4.0 (WAVES & POWERUPS)
            // ==========================================
            const canvas = document.getElementById('gameCanvas');
            const ctx = canvas.getContext('2d');

            const dpr = window.devicePixelRatio || 1;
            const rect = canvas.getBoundingClientRect();
            canvas.width = rect.width * dpr;
            canvas.height = rect.height * dpr;
            ctx.scale(dpr, dpr);
            canvas.style.width = \`\${rect.width}px\`;
            canvas.style.height = \`\${rect.height}px\`;

            // State
            let score = 0;
            let highScore = localStorage.getItem('poki_highscore_v4') || 0;
            let combo = 0;
            let comboTimer = 0;
            let maxCombo = 0;
            let gameSpeed = 1;

            // V4 New Features: Waves & Powerups
            let wave = 1;
            let particlesToClear = 50;
            let particlesCleared = 0;

            let powerupMagnet = 0; // timer
            let powerupFreeze = 0; // timer

            let particles = [];
            let explosions = [];
            let floatingTexts = [];
            let shockwaves = [];

            // Mouse
            let mouse = { x: -1000, y: -1000, vx: 0, vy: 0 };
            let lastMouse = { x: -1000, y: -1000 };

            canvas.addEventListener('mousemove', (e) => {
                const r = canvas.getBoundingClientRect();
                lastMouse.x = mouse.x;
                lastMouse.y = mouse.y;
                mouse.x = e.clientX - r.left;
                mouse.y = e.clientY - r.top;
                mouse.vx = mouse.x - lastMouse.x;
                mouse.vy = mouse.y - lastMouse.y;
            });

            canvas.addEventListener('mouseleave', () => {
                mouse.x = -1000;
                mouse.y = -1000;
            });

            const colors = [
                '#FF0099', '#493240', '#00C9FF', '#92FE9D',
                '#ff4757', '#2ed573', '#1e90ff', '#ffa502',
                '#ff6348', '#a29bfe', '#fd79a8', '#f9ca24'
            ];

            // Classes
            class Particle {
                constructor() {
                    this.x = Math.random() * (canvas.width / dpr);
                    this.y = Math.random() * (canvas.height / dpr);
                    this.vx = (Math.random() - 0.5) * 4 * gameSpeed;
                    this.vy = (Math.random() - 0.5) * 4 * gameSpeed;
                    this.radius = Math.random() * 5 + 3;
                    this.color = colors[Math.floor(Math.random() * colors.length)];

                    const rand = Math.random();
                    this.type = 'normal';

                    if (rand > 0.96) {
                        this.type = 'bomb';
                        this.color = '#fff';
                        this.radius = 8;
                    } else if (rand > 0.93) {
                        this.type = 'magnet';
                        this.color = '#9b59b6'; // purple
                        this.radius = 10;
                    } else if (rand > 0.90) {
                        this.type = 'freeze';
                        this.color = '#00ffff'; // cyan
                        this.radius = 10;
                    }
                }
                update() {
                    let speedMult = gameSpeed;
                    if (powerupFreeze > 0) speedMult *= 0.1; // Freeze time slows things down 90%

                    this.x += this.vx * speedMult;
                    this.y += this.vy * speedMult;

                    // Bounce
                    if (this.x < this.radius) { this.x = this.radius; this.vx *= -1; }
                    if (this.x > (canvas.width / dpr) - this.radius) { this.x = (canvas.width / dpr) - this.radius; this.vx *= -1; }
                    if (this.y < this.radius) { this.y = this.radius; this.vy *= -1; }
                    if (this.y > (canvas.height / dpr) - this.radius) { this.y = (canvas.height / dpr) - this.radius; this.vy *= -1; }

                    // V4 Feature: Magnet Powerup
                    if (powerupMagnet > 0 && mouse.x > 0) {
                        const dx = mouse.x - this.x;
                        const dy = mouse.y - this.y;
                        const dist = Math.sqrt(dx*dx + dy*dy);
                        if (dist < 400) {
                            this.vx += dx * 0.002;
                            this.vy += dy * 0.002;

                            // Speed cap for magnet
                            const s = Math.sqrt(this.vx*this.vx + this.vy*this.vy);
                            if(s > 10) {
                                this.vx = (this.vx/s)*10;
                                this.vy = (this.vy/s)*10;
                            }
                        }
                    } else if(this.type === 'normal') {
                        // Dodge mouse slightly
                        const dx = mouse.x - this.x;
                        const dy = mouse.y - this.y;
                        const dist = Math.sqrt(dx*dx + dy*dy);
                        if (dist < 100) {
                            this.vx -= dx * 0.005;
                            this.vy -= dy * 0.005;
                        }
                    }
                }
                draw() {
                    ctx.beginPath();

                    if (this.type === 'magnet' || this.type === 'freeze') {
                        // Star shape for powerups
                        let rot = Math.PI / 2 * 3;
                        let x = this.x; let y = this.y;
                        let step = Math.PI / 5;
                        let outerRadius = this.radius;
                        let innerRadius = this.radius / 2;

                        ctx.moveTo(this.x, this.y - outerRadius);
                        for(let i=0; i<5; i++){
                            x = this.x + Math.cos(rot)*outerRadius;
                            y = this.y + Math.sin(rot)*outerRadius;
                            ctx.lineTo(x,y);
                            rot += step;
                            x = this.x + Math.cos(rot)*innerRadius;
                            y = this.y + Math.sin(rot)*innerRadius;
                            ctx.lineTo(x,y);
                            rot += step;
                        }
                        ctx.lineTo(this.x, this.y - outerRadius);
                    } else {
                        ctx.arc(this.x, this.y, this.radius, 0, Math.PI * 2);
                    }

                    ctx.fillStyle = this.color;
                    ctx.shadowBlur = this.type !== 'normal' ? 20 : 10;
                    ctx.shadowColor = this.color;
                    ctx.fill();
                    ctx.closePath();

                    if (this.type === 'bomb') {
                        ctx.strokeStyle = '#ff4757';
                        ctx.lineWidth = 2;
                        ctx.stroke();
                    } else if (this.type === 'magnet') {
                        ctx.strokeStyle = '#fff';
                        ctx.lineWidth = 1;
                        ctx.stroke();
                    }
                    ctx.shadowBlur = 0;
                }
            }

            class Shockwave {
                constructor(x, y, color) {
                    this.x = x; this.y = y; this.color = color; this.radius = 0; this.maxRadius = 150; this.life = 1;
                }
                update() { this.radius += 8; this.life -= 0.04; }
                draw() {
                    ctx.beginPath();
                    ctx.arc(this.x, this.y, this.radius, 0, Math.PI * 2);
                    ctx.strokeStyle = \`\${this.color.replace(')', \`,\${this.life})\`).replace('rgb', 'rgba')}\`;
                    ctx.lineWidth = 4 * this.life;
                    ctx.stroke();
                    ctx.closePath();
                }
            }

            class FloatingText {
                constructor(x, y, text, color, scale = 1) {
                    this.x = x; this.y = y; this.text = text; this.color = color; this.life = 1.0; this.vy = -2; this.scale = scale;
                }
                update() { this.y += this.vy; this.life -= 0.02; }
                draw() {
                    ctx.fillStyle = \`rgba(255,255,255,\${this.life})\`;
                    ctx.font = \`bold \${20 * this.scale}px "Fredoka One"\`;
                    ctx.textAlign = 'center';
                    ctx.shadowBlur = 5;
                    ctx.shadowColor = this.color;
                    ctx.fillText(this.text, this.x, this.y);
                    ctx.shadowBlur = 0;
                }
            }

            function createExplosion(x, y, color, isBig = false) {
                const count = isBig ? 30 : 10;
                for(let i=0; i<count; i++) {
                    explosions.push({
                        x: x, y: y,
                        vx: (Math.random() - 0.5) * (isBig ? 18 : 8),
                        vy: (Math.random() - 0.5) * (isBig ? 18 : 8),
                        life: 1.0, color: color
                    });
                }
                shockwaves.push(new Shockwave(x, y, color === '#fff' ? 'rgb(255,255,255)' : 'rgb(0,255,255)'));
            }

            // Init Wave 1
            for (let i = 0; i < 70; i++) particles.push(new Particle());

            function nextWave() {
                wave++;
                floatingTexts.push(new FloatingText(canvas.width/(2*dpr), canvas.height/(2*dpr), \`WAVE \${wave}!\`, '#f1c40f', 2));
                particlesToClear = 50 + (wave * 20);
                particlesCleared = 0;
                gameSpeed += 0.2;

                // Spawn wave
                for (let i = 0; i < 70 + (wave * 5); i++) {
                    setTimeout(() => particles.push(new Particle()), Math.random() * 2000);
                }
            }

            // Interaction
            canvas.addEventListener('mousedown', (e) => {
                const r = canvas.getBoundingClientRect();
                const mx = e.clientX - r.left;
                const my = e.clientY - r.top;

                let hit = false;
                for (let i = particles.length - 1; i >= 0; i--) {
                    const p = particles[i];
                    const dx = mx - p.x;
                    const dy = my - p.y;
                    const dist = Math.sqrt(dx*dx + dy*dy);

                    if (dist <= p.radius * 3.5) { // generous hit box
                        if (p.type === 'bomb') {
                            createExplosion(p.x, p.y, p.color, true);
                            floatingTexts.push(new FloatingText(p.x, p.y, "BOOM!", "#fff", 1.5));
                            particles.splice(i, 1);

                            // Find nearby
                            for(let j = particles.length - 1; j >= 0; j--) {
                                const bp = particles[j];
                                const bdx = p.x - bp.x;
                                const bdy = p.y - bp.y;
                                if(Math.sqrt(bdx*bdx + bdy*bdy) < 200) {
                                    createExplosion(bp.x, bp.y, bp.color);
                                    score += 20 * combo;
                                    particles.splice(j, 1);
                                    particlesCleared++;
                                }
                            }
                            hit = true;
                            combo += 5;
                        } else if (p.type === 'magnet') {
                            createExplosion(p.x, p.y, p.color);
                            floatingTexts.push(new FloatingText(p.x, p.y, "MAGNET!", p.color, 1.5));
                            powerupMagnet = 600; // 10 seconds approx
                            particles.splice(i, 1);
                            hit = true;
                            combo++;
                            particlesCleared++;
                        } else if (p.type === 'freeze') {
                            createExplosion(p.x, p.y, p.color);
                            floatingTexts.push(new FloatingText(p.x, p.y, "TIME FREEZE!", p.color, 1.5));
                            powerupFreeze = 300; // 5 seconds approx
                            particles.splice(i, 1);
                            hit = true;
                            combo++;
                            particlesCleared++;
                        } else {
                            createExplosion(p.x, p.y, p.color);
                            floatingTexts.push(new FloatingText(p.x, p.y, \`+\${10 * (combo+1)}\`, p.color));
                            particles.splice(i, 1);
                            hit = true;
                            combo++;
                            score += 10 * combo;
                            particlesCleared++;
                            // Respawn 1 for 1 unless we are clearing the wave end
                            if(particles.length < 30) particles.push(new Particle());
                        }

                        comboTimer = 120;
                        if(combo > maxCombo) maxCombo = combo;

                        if (window.userSystem) {
                            window.userSystem.addXP(Math.min(combo, 15));
                        }
                    }
                }

                if (!hit) {
                    combo = 0;
                    floatingTexts.push(new FloatingText(mx, my, "Miss!", "#ff4757"));
                } else {
                    if (score > highScore) {
                        highScore = score;
                        localStorage.setItem('poki_highscore_v4', highScore);
                    }
                    if (particlesCleared >= particlesToClear) {
                        nextWave();
                    }
                }
            });

            // Parallax Stars
            let stars = Array.from({length: 80}, () => ({
                x: Math.random() * (canvas.width/dpr),
                y: Math.random() * (canvas.height/dpr),
                s: Math.random() * 3 + 0.5,
                c: \`hsl(\${Math.random()*360}, 80%, 80%)\`
            }));

            function animate() {
                if(powerupFreeze > 0) {
                    ctx.fillStyle = 'rgba(10, 20, 35, 0.5)'; // bluish freeze tint
                    powerupFreeze--;
                } else if(powerupMagnet > 0) {
                    ctx.fillStyle = 'rgba(25, 10, 35, 0.3)'; // purple magnet tint
                    powerupMagnet--;
                } else {
                    ctx.fillStyle = 'rgba(10, 14, 25, 0.35)'; // normal trail
                }
                ctx.fillRect(0, 0, canvas.width / dpr, canvas.height / dpr);

                stars.forEach(s => {
                    let speed = s.s * 0.5 * gameSpeed;
                    if(powerupFreeze > 0) speed *= 0.1;
                    s.y += speed;
                    if(s.y > canvas.height/dpr) s.y = 0;
                    ctx.fillStyle = s.c;
                    ctx.fillRect(s.x, s.y, s.s, s.s);
                });

                // Draw connecting lines
                for(let i=0; i<particles.length; i++) {
                    for(let j=i+1; j<particles.length; j++) {
                        const dx = particles[i].x - particles[j].x;
                        const dy = particles[i].y - particles[j].y;
                        const dist = dx*dx + dy*dy;

                        if(dist < 10000) {
                            ctx.beginPath();
                            ctx.moveTo(particles[i].x, particles[i].y);
                            ctx.lineTo(particles[j].x, particles[j].y);
                            ctx.strokeStyle = \`rgba(0, 255, 255, \${0.25 - dist/40000})\`;
                            ctx.lineWidth = powerupMagnet > 0 ? 2 : 1;
                            ctx.stroke();
                        }
                    }
                }

                particles.forEach(p => { p.update(); p.draw(); });

                for(let i = shockwaves.length - 1; i >= 0; i--) {
                    shockwaves[i].update();
                    shockwaves[i].draw();
                    if(shockwaves[i].life <= 0) shockwaves.splice(i, 1);
                }

                for(let i = explosions.length - 1; i >= 0; i--) {
                    let ex = explosions[i];
                    let speed = 1;
                    if(powerupFreeze > 0) speed = 0.2;
                    ex.x += ex.vx * speed;
                    ex.y += ex.vy * speed;
                    ex.life -= 0.03 * speed;
                    if(ex.life <= 0) { explosions.splice(i, 1); continue; }

                    ctx.beginPath();
                    ctx.arc(ex.x, ex.y, 3 * ex.life, 0, Math.PI*2);
                    ctx.fillStyle = \`rgba(255, 255, 255, \${ex.life})\`;
                    ctx.shadowBlur = 10;
                    ctx.shadowColor = ex.color;
                    ctx.fill();
                    ctx.closePath();
                    ctx.shadowBlur = 0;
                }

                for(let i = floatingTexts.length - 1; i >= 0; i--) {
                    let speed = 1;
                    if(powerupFreeze > 0) speed = 0.5;
                    floatingTexts[i].y += floatingTexts[i].vy * speed;
                    floatingTexts[i].life -= 0.02 * speed;
                    floatingTexts[i].draw();
                    if(floatingTexts[i].life <= 0) floatingTexts.splice(i, 1);
                }

                // V4 UI Overlays
                ctx.fillStyle = 'white';
                ctx.font = 'bold 24px "Fredoka One", cursive';
                ctx.textAlign = 'left';
                ctx.fillText(\`Punkte: \${score}\`, 20, 40);

                ctx.font = 'bold 16px "Nunito"';
                ctx.fillStyle = '#95a5a6';
                ctx.fillText(\`High Score: \${highScore} | Max Combo: \${maxCombo}\`, 20, 65);

                // Wave Progress Bar
                ctx.fillStyle = '#2c3e50';
                ctx.fillRect(20, 80, 200, 10);
                ctx.fillStyle = '#f1c40f';
                ctx.fillRect(20, 80, 200 * (particlesCleared / particlesToClear), 10);
                ctx.fillStyle = 'white';
                ctx.fillText(\`WAVE \${wave}\`, 230, 90);

                if (combo > 1 && comboTimer > 0) {
                    comboTimer--;
                    const scale = 1 + (comboTimer/120) * 0.5;
                    ctx.save();
                    ctx.translate(20, 130);
                    ctx.scale(scale, scale);
                    ctx.fillStyle = \`hsla(\${combo * 10}, 100%, 60%, \${comboTimer/120})\`;
                    ctx.font = 'bold 32px "Fredoka One", cursive';
                    ctx.shadowBlur = 10;
                    ctx.shadowColor = ctx.fillStyle;
                    ctx.fillText(\`\${combo}x COMBO!\`, 0, 0);
                    ctx.restore();
                } else if (comboTimer <= 0) {
                    combo = 0;
                }

                // Powerup Timers
                if(powerupMagnet > 0) {
                    ctx.fillStyle = '#9b59b6';
                    ctx.fillText(\`MAGNET: \${Math.ceil(powerupMagnet/60)}s\`, 20, canvas.height/dpr - 40);
                }
                if(powerupFreeze > 0) {
                    ctx.fillStyle = '#00ffff';
                    ctx.fillText(\`FREEZE: \${Math.ceil(powerupFreeze/60)}s\`, 20, canvas.height/dpr - 20);
                }

                requestAnimationFrame(animate);
            }
            animate();
`;

let upgradedCount = 0;

games.forEach(gameFile => {
    const filePath = path.join(gamesDir, gameFile);
    let content = fs.readFileSync(filePath, 'utf-8');

    // We only want to inject this into the 2D games, NOT the 3D ones.
    // 3D games use <script src="../js/engine3d.js"></script>
    if (!content.includes('engine3d.js')) {
        const regex = /const canvas = document\.getElementById\('gameCanvas'\);[\s\S]*?(?=\}\);?\s*<\/script>)/g;
        if (regex.test(content)) {
            content = content.replace(regex, v4Script);
            fs.writeFileSync(filePath, content);
            upgradedCount++;
        }
    }
});

console.log(`Successfully injected V4 mechanics (Waves & Powerups) into ${upgradedCount} 2D games.`);
